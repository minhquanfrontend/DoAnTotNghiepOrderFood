...
def create_payment(request):
    ...
    if payment_method == 'cash':
        ...
        # COD orders go directly to pending (waiting for seller)
        ...
        # VNPay orders start as unpaid (waiting for payment)
        order.status = 'unpaid'
        order.payment_status = 'pending'
        order.save()
...
def get_payment_methods(request):
    ...
    return Response({'success': True, 'methods': methods})
...
def vnpay_callback(request):
    ...
    try:
        ...
        if vnp_response_code == '00':
            ...
            # Update order - move from unpaid to pending (waiting for seller)
            payment.order.payment_status = 'paid'
            payment.order.status = 'pending'  # Now waiting for seller confirmation
            payment.order.save()
            ...
            # Payment failed - keep order in unpaid status
            payment.status = 'failed'
            payment.gateway_response = dict(data)
            payment.save()
            
            # Order stays in unpaid status
            payment.order.payment_status = 'failed'
            payment.order.status = 'unpaid'
            payment.order.save()
